...
from typing import Sequence, LiteralString
...
def insert_one_item(i: Item, user_id: int) -> int | None:
    ...
    sql: LiteralString = """ INSERT INTO item (name, normalized_name, created_by) VALUES (%s,%s, %s) RETURNING id"""
...
def get_all_items_name_match(name: str) -> Sequence[Item]:
    ...
    sql: LiteralString = """ SELECT id, name FROM item WHERE name ILIKE %s """
    values = (f"{name}%",)

    return select_sql(sql, values, Item)
...
if __name__ == "__main__":
    ...
    print(get_all_items_name_match("roas"))